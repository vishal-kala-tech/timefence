...
def day_policy(resource, now=None):
    now=now or datetime.now(); key="weekend" if now.weekday() >= 5 else "weekday"
...
def in_window(window, now=None):
    now=now or datetime.now(); cur=now.hour*60+now.minute; start=_minutes(window["start"]); end=_minutes(window["end"])
